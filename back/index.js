...
app.post('/itens', (req, res) => {
  ...
  if (!personagem_id || !nome_item || !quantidade) {
    return res.status(400).send("Personagem, nome do item e quantidade inicial são obrigatórios.");
  }
...
app.get('/personagens/:personagem_id', (req, res) => {
    ...
    db.all( "SELECT id, nome, dinheiro FROM personagens WHERE personagem_id = ?",
    [personagem_id],
    (err, rows) => {
...
app.put('/personagens/:id/dinheiro', (req, res) => {
  ...
  db.get("SELECT dinheiro FROM personagens WHERE id = ?", [id], (err, row) => {
    ...
    db.run("UPDATE personagens SET dinheiro = ? WHERE id = ?", [novoDinheiro, id], function (err) {
